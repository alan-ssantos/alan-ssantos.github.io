...
Tunnel.prototype.init = function() {
  ...
  this.camera = new THREE.PerspectiveCamera(8, ww / wh, 0.01, 10);
  // this.camera.rotation.y = Math.PI;
  this.camera.position.z = 0.4;
  
  this.scene = new THREE.Scene();
  this.scene.fog = new THREE.Fog(0x00eeff, 1, 1.9);
};
...
Tunnel.prototype.createMesh = function() {
  ...
  for (i = 0; i < 5; i += 1) {
    points.push(new THREE.Vector3(0, 0, 3 * (i / 1)));
  }
  points[4].y = -0.06;
  ...
  this.tubeGeometry = new THREE.TubeGeometry(this.curve, 2000, 0.02, 3, false);
  ...
  for (var i = 0; i < this.tubeGeometry.faces.length; i++) {
    ...
    color = new THREE.Color(
      "hsl(" +
      (Math.floor(
        Math.abs(noise.simplex3(p.x * 2, p.y * 4, p.z * 2)) * 80 * 100) * 0.01 + 180) + ",70%,60%)");
        f.color = color;
